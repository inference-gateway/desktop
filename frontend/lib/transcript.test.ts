...
test("TokenUsage accumulates per conversation and resets on newChat", () => {
  const usage = (input: number, output: number) =>
    ev({ kind: "TokenUsage", input, output, cached_read: 1, total_tool_calls: 1 });
  const a = run([usage(10, 5), usage(1, 1)]);
  const b = run([usage(100, 50)]);
  expect(a.usage).toEqual({ input: 11, output: 6, cached_read: 2, total_tool_calls: 2 });
