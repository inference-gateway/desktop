...
// `!!` tools mode: parse `!!ToolName(arg="value")` as typed in the composer.
...
export function parseToolCall(text: string): ParsedToolCall {
  const bad = (why: string): ParsedToolCall => ({
    ok: false,
  ...
  if (inner) {
    if (inner.startsWith("{")) {
      ...
      let consumed = 0;
      for (const g of inner.matchAll(re)) {
        const between = inner.slice(consumed, g.index).trim();
        if (between && between !== ",") return bad("unexpected text -");
        try {
          args[g[1]] = JSON.parse(g[2]);
        } catch {
          return bad("invalid argument value -");
        }
        consumed = g.index + g[0].length;
      }
      if (inner.slice(consumed).trim()) return bad("unexpected text -");
    }
  }
  return { ok: true, name: m[1], args };
}
