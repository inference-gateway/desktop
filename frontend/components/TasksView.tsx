...
// Long-horizon tasks: create GitHub issues the installed infer-action
// workflow picks up, and watch the resulting issues and workflow runs.
export function TasksView() {
  const { setCurrentView, openSettings, setInitialSettingsTab } = useDesktop();
  ...
  const openGithubSettings = () => {
    setInitialSettingsTab("github");
    openSettings();
  };

  return (
    <div id="tasks-view" className="flex min-h-0 flex-1 flex-col overflow-y-auto p-5">
      <div className="mb-4 flex items-center gap-3">
        <button
          onClick={() => setCurrentView("chat")}
          className="inline-flex items-center gap-1.5 rounded-md px-2 py-[0.45rem] text-[0.85rem] font-medium text-muted-foreground hover:bg-primary/10 hover:text-foreground"
        >
          <ArrowLeft size={15} /> Back
        </button>
        <h2 className="text-[1.05rem] font-semibold">Tasks</h2>
        {repo && (
          <span className="text-[0.8rem] text-muted-foreground">{repo}</span>
        )}
        <Button
          variant="ghost"
          size="icon-sm"
      ...
      {loaded && !repo && (
        <p className="text-[0.85rem] text-muted-foreground">
          ...
          <button onClick={openGithubSettings} className="text-primary underline">
            Pick one in Settings → GitHub
          </button>{" "}
      ...
      {repo && (
        <>
          <div className="mb-6 flex max-w-[640px] flex-col gap-2">
          ...
          <div className="grid max-w-[1100px] gap-6 md:grid-cols-2">
            <section>
